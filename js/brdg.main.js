...
var xhr;
var STR_CITYLABEL = new Array('China','Other countries','china<br/>other countries');
var citylabel = 2;
var fireOnce = true;
var autoSlide;
...
function brdgInit() {
   ...
   initInfinitescroll(siteurl+'data/third_content/all_resources?perpage=' + perpage + '&page=');
    ...
    $('.popup_content1 .popup_close').on('click', function(){
        $('.popup_content').fadeOut();
    });

    $('.open_popup').on('click', function(){
        $('.popup_content1').fadeIn();
    });

    $('.popup_content2 .popup_close').on('click', function(){
        $('.popup_content2').fadeOut();
    });

    $('.open_popup2').on('click', function(){
        $('.popup_content2').fadeIn();
    });
}
...
function renderItem(data, type) {
    for (var i in data) {
        if(!_.isEmpty(data[i]))
        {
            ...
            // Get user city
            var user_city = data[i].field_content_user_city;
            if (user_city === '1') {
                user_city = "Other countries";
            }
            else if(user_city === '2')
            {
                user_city = "New York";
            ...
            var date_media_date = new Date(str_media_date);
             if(data[i].field_content_user_city === '1'){
                var parisTimeZone = 7;
                if (isBSTinEffect())
                    parisTimeZone = 6;
                date_media_date = new Date(date_media_date.getTime() - parisTimeZone * 60 * 60 * 1000);
            }
			if(data[i].field_content_user_city === '2'){
				var nykTimeZone = -12;
				date_media_date = new Date(date_media_date.getTime() - nykTimeZone * 60 * 60 * 1000);
			}
...
function loadSingleContent(nid, isOpen) {
    ...
    $_node.getPreNextNodes(nid, 10, 0, current_cityid, 0, function (data) {
        clickable = true;
        $('.loading2').fadeOut();
...
function renderMediaContent(media_data, face, noimg) {
    ...
    var date_media_date = new Date(str_media_date);
    if(media_data.field_content_user_city === '1'){
        var parisTimeZone = 7;
        if (isBSTinEffect())
            parisTimeZone = 6;
        date_media_date = new Date(date_media_date.getTime() - parisTimeZone * 60 * 60 * 1000);
    }
	if(media_data.field_content_user_city === '2'){
		var nykTimeZone = -13;
		date_media_date = new Date(date_media_date.getTime() - nykTimeZone * 60 * 60 * 1000);
	}
    // Covert Timezone
    ...
    else if(media_data.field_content_user_city == '2')
    {
        city = 'New York';
    }
    else if(media_data.field_content_user_city == '3')
    {
        city = 'Beijing';
    }
	else {
    ...
    $('.dc1_face' + face + ' .date').html($_utility.formateNumber(media_date_month) + '/' + media_date_year);
    $('.dc1_face' + face + ' .time_clock').attr('data-hrs', media_date_hrs);
    $('.dc1_face' + face + ' .time_clock').attr('data-min', media_date_min);
    $('.dc1_face' + face + ' .time_text').html($_utility.formateNumber(media_date_hrs) + ' : ' + $_utility.formateNumber(media_date_min));
    $('.dc1_face' + face + ' .time_clock').brdgClock();
    $('.dc1_face' + face + ' .source').html('').append('<div>#worldofdiesel</div>');
...
function bindDcEventOnce() {
    ...
    $('.category li').eq(1).click(function () {
        ...
        citylabel = 2;
        openCategoryOverlay();
        loadData('data/third_content/img_api?perpage=' + perpage + '&page=0',1);
        initInfinitescroll(siteurl + 'data/third_content/img_api?perpage='+ perpage +'&page=');
    });
    ...
    $('.category li').eq(2).click(function () {
        ...
        citylabel = 2;
        openCategoryOverlay();
        loadData('data/third_content/video_api?perpage=' + perpage + '&page=0');
        initInfinitescroll(siteurl + 'data/third_content/video_api?perpage=' + perpage + '&page=');
    });
    //filter music
...
function resizeWindow() {
    ...
    var dc2_section1_height = $(window).height() - $('.dc2_header').height()-3;
...
function bindTextClockEvent() {
    ...
    if (isBSTinEffect())
        parisTimeZone = 2;
    $('#paris_time').textClock(parisTimeZone, {}, function(h,m) {
        $('#paris_clock').attr('data-hrs', h);
        $('#paris_clock').attr('data-min', m);
        $('#paris_clock').brdgClock();
    });
    $('#shanghai_time').textClock(8, {}, function(h,m) {
        $('#shanghai_clock').attr('data-hrs', h);
        $('#shanghai_clock').attr('data-min', m);
        $('#shanghai_clock').brdgClock();
    });
    $('#beijing_time').textClock(8);
	$('#nyk_time').textClock(-4);
